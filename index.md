...
$$
\large J(\theta^\mu) = \mathbb{E}_{s_t \sim D} \left[ Q(s_t, \mu(s_t|\theta^\mu)) \right]
$$
...
$$
where $\large \tau$ is a small constant typically around 0.001
...
$$
   \large \mu'(s_t) = \mu(s_t|\theta^\mu) + N_t​
$$
...
$$
where:
- $\large x_t$ → the process value at time t
- $\large \theta$ → the rate of mean reversion
- $\large \mu$ → the long-term mean
- $\large \sigma$ → the volatility (noise amplitude)
- $\large W_t$​ → a Wiener process (standard Brownian motion)

In reinforcement learning we use the discrete form:
$$
\large x_{t+1} = x_t + \theta (\mu - x_t) \Delta t + \sigma \sqrt{\Delta t} \, \epsilon_t, \quad \epsilon_t \sim \mathcal{N}(0, 1)
$$
```python
class OUNoise:
